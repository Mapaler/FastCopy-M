...
//		| (FastCopy::REPORT_ACL_ERROR | FastCopy::REPORT_STREAM_ERROR)
	...
	if ((ret || is_initerr_logging) && (pathLogBuf = new char [pathLogMax])) {
		int len = sprintf(pathLogBuf, "<%s> %s", GetLoadStr(IDS_Log0), isUtf8Log ? WtoU8(src) : IS_WINNT_V ?
					WtoA(src) : (char *)src);
		if (!is_delete_mode) {
			len += sprintf(pathLogBuf + len, "\r\n<%s> %s", GetLoadStr(IDS_Log1), isUtf8Log ?
					WtoU8(dst) : IS_WINNT_V ? WtoA(dst) : (char *)dst);
		}
		if (inc && GetChar(inc, 0)) {
			len += sprintf(pathLogBuf + len, "\r\n<%s> %s", GetLoadStr(IDS_Log2), isUtf8Log ?
					WtoU8(inc) : IS_WINNT_V ? WtoA(inc) : (char *)inc);
		}
		if (exc && GetChar(exc, 0)) {
			len += sprintf(pathLogBuf + len, "\r\n<%s> %s", GetLoadStr(IDS_Log3), isUtf8Log ?
					WtoU8(exc) : IS_WINNT_V ? WtoA(exc) : (char *)exc);
		}
		len += sprintf(pathLogBuf + len, "\r\n<%s> %s", GetLoadStr(IDS_Log4), isUtf8Log ?
		...
		if (info.flags & (FastCopy::WITH_ACL|FastCopy::WITH_ALTSTREAM|FastCopy::OVERWRITE_DELETE
			...
			len += sprintf(pathLogBuf + len, " (%s", GetLoadStr(IDS_Log5));
			if (!is_delete_mode && (info.flags & FastCopy::VERIFY_FILE))
				len += sprintf(pathLogBuf + len, " %s", GetLoadStr(IDS_Log6));
			if (!is_delete_mode && (info.flags & FastCopy::WITH_ACL))
				len += sprintf(pathLogBuf + len, " %s", GetLoadStr(IDS_Log7));
			if (!is_delete_mode && (info.flags & FastCopy::WITH_ALTSTREAM))
				len += sprintf(pathLogBuf + len, " %s", GetLoadStr(IDS_Log8));
			if (is_delete_mode && (info.flags & FastCopy::OVERWRITE_DELETE))
				len += sprintf(pathLogBuf + len, " %s", GetLoadStr(IDS_Log9));
			if (is_delete_mode && (info.flags & FastCopy::OVERWRITE_DELETE_NSA))
				len += sprintf(pathLogBuf + len, " %s", GetLoadStr(IDS_Log10));
			len += sprintf(pathLogBuf + len, ")");
		}
		len += sprintf(pathLogBuf + len , "\r\n");
	}
...
BOOL TMainDlg::EndCopy(void)
{
	...
	if (is_starting) {
		...
		SendDlgItemMessage(STATUS_EDIT, WM_SETTEXT, 0, (LPARAM)GetLoadStr(IDS_EndCopy0));
	}
	...
	if (GetChar(act->command, 0) &&
		...
		for (int i=0; i < pathArray.Num(); i++) {
			...
			if (ShellExecuteExV(&sei) && is_wait && sei.hProcess) {
				...
				_snprintf(buf, sizeof(buf), "%s\r\n%s", svbuf,GetLoadStr(IDS_FinalAction0));
...
void TMainDlg::WriteLogHeader(HANDLE hFile, BOOL add_filelog)
{
	...
	DWORD len = sprintf(buf, "-------------------------------------------------\r\n"
		...
		GetVersionStr(), GetVerAdminStr(), GetLoadStr(IDS_LogHeader0),
	...
	if (add_filelog && *fileLogPathV) {
		len = sprintf(buf, "<%s> %s\r\n",GetLoadStr(IDS_LogHeader1), IS_WINNT_V ? cfg.isUtf8Log ?
	...
	if (finActIdx >= 1) {
		...
		len = sprintf(buf, "<%s> %s\r\n\r\n",GetLoadStr(IDS_LogHeader2), IS_WINNT_V ? cfg.isUtf8Log ?
...
BOOL TMainDlg::WriteErrLog(BOOL is_initerr)
{
	...
	if (errBufOffset) {
		...
		char *msg = GetLoadStr(IDS_ErrLog0);
				...
				int remain_h, int remain_m, int remain_s)
{
	...
	if (!cfg.taskbarMode) {
		if (info.mode == FastCopy::DELETE_MODE) {
			...
			&& doneRate >= 0.0001) {
				len += sprintf(buf + len, "%d%% (%s %02d:%02d:%02d) ",
					doneRatePercent,
		...
		if (is_finish_status) {
			sprintf(buf + len, " %s",
				GetLoadStr(IDS_TaskTrayInfo_Finished)
			);
		}
	}
...
BOOL TMainDlg::SetInfo(BOOL is_finish_status)
{
	...
	if ((info.flags & FastCopy::PRE_SEARCH) && !ti.total.isPreSearch && !is_finish_status
	&& doneRate >= 0.0001) {
		len += sprintf(buf + len, " -- %s %02d:%02d:%02d (%d%%) --\r\n",
				GetLoadStr(IDS_Info_Remain),remain_h, remain_m, remain_s, doneRatePercent);
	}
	if (ti.total.isPreSearch) {
		...
		if (IsListing()) {
			len += sprintf(buf + len,
				...
				"%s = %.1f MB\r\n"
				"%s = %d (%d)\r\n"
				, GetLoadStr(IDS_Info_TotalSize)
				, (double)ti.total.writeTrans / (1024 * 1024)
				, GetLoadStr(IDS_Info_TotalFiles)
				, ti.total.writeFiles
				, (info.flags & FastCopy::RESTORE_HARDLINK) ?
				  ti.total.linkFiles :
				  ti.total.writeDirs
			...
			len += sprintf(buf + len,
				...
				"%s = %.1f MB\r\n"
				"%s = %d (%d)\r\n"
				, GetLoadStr(IDS_Info_TotalRead)
				, (double)ti.total.readTrans / (1024 * 1024)
				, GetLoadStr(IDS_Info_TotalWrite)
				, (double)ti.total.writeTrans / (1024 * 1024)
				, GetLoadStr(IDS_Info_TotalFiles)
				, ti.total.writeFiles
				, (info.flags & FastCopy::RESTORE_HARDLINK) ?
...
//	SetDlgItemText(SPEED_STATIC, buf);
	...
	if (ti.errBuf->UsedSize() > errBufOffset || errBufOffset == MAX_ERR_BUF || is_finish_status) {
		...
		sprintf(buf, "(%s : %d / %s : %d)", GetLoadStr(IDS_Info_ErrFiles),ti.total.errFiles, GetLoadStr(IDS_Info_ErrDirs),ti.total.errDirs);
